...
#include  <limits.h>			/* Standard I/O definitions. */
...
#define def_N_elem_simple_queue 16
#define def_size_elem_simple_queue 1024
typedef struct
{
	unsigned int nw,nr;
	unsigned int iw,ir;
	char queue[def_N_elem_simple_queue][def_size_elem_simple_queue];
}type_simple_queue;
static type_simple_queue simple_queue ={0};
static unsigned int n_elem_simple_queue(void)
{
	return simple_queue.nw-simple_queue.nr;
}
unsigned int is_empty_simple_queue(void)
{
	return n_elem_simple_queue() == 0 ? 1: 0;
}
unsigned int is_full_simple_queue(void)
{
	return n_elem_simple_queue() >= def_N_elem_simple_queue ? 1: 0;
}
unsigned int is_OK_push_simple_queue(char *s, unsigned int n)
{
	if (is_full_simple_queue())
	{
		return 0;
	}
	unsigned int iw = simple_queue.iw;
	if (iw >= def_N_elem_simple_queue)
	{
		iw = 0;
	}
	if (n > def_size_elem_simple_queue)
	{
		n = def_size_elem_simple_queue;
	}
	memcpy(&simple_queue.queue[iw][0],s,n);
	if (++iw >= def_N_elem_simple_queue)
	{
		iw = 0;
	}
...
{
	if (is_empty_simple_queue())
	{
		return 0;
	}
	unsigned int ir = simple_queue.ir;
	if (ir >= def_N_elem_simple_queue)
	{
		ir = 0;
	}
	if (n > def_size_elem_simple_queue)
	...
	memcpy(s,&simple_queue.queue[ir][0],n);
	if (++ir >= def_N_elem_simple_queue)
	{
		ir = 0;
	}
